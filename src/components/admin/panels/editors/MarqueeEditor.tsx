...
export default function MarqueeEditor({ page, title, description }: { page: "home" | "artiste" | "entreprise"; title: string; description?: string }) {
  return (
    <TableEditor
      ...
      initialRecord={{ page, kind: "word", text_value: "", image_url: "" }}
      fields={[
        { key: "kind", label: "Type", type: "select", options: [
          { label: "Mot / texte", value: "word" },
          { label: "Logo (image)", value: "logo" },
        ]},
        { key: "text_value", label: "Texte" },
        { key: "image_url", label: "Image / logo", type: "image", imageFolder: "marquee", hint: "Si type = logo" },
      ...
      renderItem={(item) => (
        <div className="flex items-center gap-2">
          ...
          {item.image_url && <img src={item.image_url} alt="" className="h-6 max-w-[80px] object-contain" />}
